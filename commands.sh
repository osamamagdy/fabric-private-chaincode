# # - You don't need to start both networks
# # - You only need to start fpc network 
# # - from outside build and package the cc-tools code the normal way
# #    but in the middle follow the fpc guidelines
# # - Then take the backaged chaincode and deploy it to the fpc network
# # See if everything works like fpc mentioned
...
# #################For CC-tools
# # After editting the code and adding the package to the imports 
# # run this command to update all packages inside the chaincode directory: 

# go get -u ./...

# # Then from the cc-tools directory package the chaincode as a tar 
...
# - comment the "RUN_CCAAS" Logic or set it the same way as "CHAINCODE_PKG_ID"
...
# # go get github.com/hyperledger-labs/cc-tools
# # go get github.com/hyperledger-labs/cc-tools/assets@v1.0.0
# # go mod vendor
# #   nano vendor/github.com/hyperledger-labs/cc-tools/mock/mockstub.go
# # 	// PurgePrivateData ...
# # 	func (stub *MockStub) PurgePrivateData(collection, key string) error {
# # 		return errors.New("Not Implemented")
# # 	}
